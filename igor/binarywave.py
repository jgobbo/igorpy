...
complexUInt16 = _numpy.dtype([("real", _numpy.uint16), ("imag", _numpy.uint16)])
complexUInt32 = _numpy.dtype([("real", _numpy.uint32), ("imag", _numpy.uint32)])
...
BinHeader2 = _Structure(  # `version` field pulled out into Wave
    ...
    fields=[
        ...
        _Field(
            "l", "pictSize", default=0, help="Reserved. Write zero. Ignore on read."
        ),
        _Field("h", "checksum", help="Checksum over this header and the wave header."),
    ],
)
...
WaveHeader2 = _DynamicStructure(
    ...
    fields=[
        _Field(
            "h", "type", help="See types (e.g. NT_FP64) above. Zero for text waves."
        ),
        _Field(
            "P",
            "next",
        ...
        _Field(
            "c", "kindBits", default=0, help="Reserved. Write zero. Ignore on read."
        ),
        _Field(
            "P",
            "formula",
...
class DynamicWaveDataField1(_DynamicField):
    ...
    def pre_unpack(self, parents, data):
        ...
        # integers.
        self.dtype = _numpy.dtype(type_).newbyteorder(wave_structure.byte_order)
        if (
            version == 3
            and self.count > 0
    ...
    def unpack(self, stream):
        ...
        try:
            ...
            _LOG.error(
                "could not reshape data from {} to {}".format(self.shape, data_b)
            )
            raise
        return data
